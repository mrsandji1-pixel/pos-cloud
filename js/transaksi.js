...
async function tambahProdukKeCart(barcode) {
  ...
  if (!p) {
    ...
    p = all.find(x =>
      ...
      barcode.includes(x.barcode) ||
      x.nama.toLowerCase().includes(barcode.toLowerCase())
    );
  }
...
function renderCart() {
  ...
  cart.forEach((item, idx) => {
    ...
    row.innerHTML = `
      ...
      <td>Rp${sub.toLocaleString('id')}</td>
      <td>
...
async function bayarDanCetak() {
  ...
  const trx = {
    ...
    customer: cust,
    items: cart.map(i => ({ barcode: i.barcode, nama: i.nama, harga: i.harga, qty: i.qty, subtotal: i.harga * i.qty })),
    total, bayar, kembali
  };
  ...
  try {
    ...
    // Simpan ke folder lokal (opsional)
    if (workingDirHandle) {
      try {
    ...
    // Cetak via Bluetooth atau fallback PDF
