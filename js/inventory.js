...
function isiFormProduk(produk, isNew, isAdmin) {
  ...
  if (isAdmin) {
    ...
    // ========== BAGIAN SIMPAN (DIUBAH) ==========
    document.getElementById('btnSimpanProduk').onclick = async () => {
      if (!currentBarcode) return;

      let foto = produk.foto || null; // URL lama jika ada

      if (fotoDihapus) {
        // Hapus foto lama dari storage
        if (foto) await removeProductPhoto(foto);
        foto = null;
      } else {
        ...
        if (fi.files[0]) {
          ...
          try {
            foto = await uploadProductPhoto(fi.files[0]);
          } catch (e) {
            alert('Gagal upload foto: ' + e.message);
            return;
          }
        }
        // Jika tidak ada file baru, pertahankan foto lama (URL)
      }
      ...
      const data = {
        ...
        foto: foto // sekarang berupa URL publik, bukan base64
      };
    ...
    document.getElementById('btnHapusProduk').onclick = async () => {
      if (confirm('Hapus produk ini?')) {
        ...
        if (produk.foto) await removeProductPhoto(produk.foto);
...
async function refreshProductList() {
  ...
  tbody.innerHTML = '';
  if (!all.length) {
    tbody.innerHTML = '<tr><td colspan="7">Belum ada</td></tr>';
    return;
  }
...
async function hapusProdukDariDaftar(b) {
  ...
  const prod = await getProductByBarcode(b);
  if (prod && prod.foto) await removeProductPhoto(prod.foto);
