...
function setupTransaksi() {
  ...
  document.getElementById('scanInputTrans').onkeydown = e => {
    if (e.key === 'Enter') {
      ...
      if (b) { e.target.value = ''; tambahProdukKeCart(b); }
    }
  };
}
...
async function searchProductFn(query) {
  ...
  div.querySelectorAll('.search-item[data-barcode]').forEach(i => {
    i.onclick = () => { div.style.display='none'; document.getElementById('searchProduct').value=''; tambahProdukKeCart(i.dataset.barcode); };
  });
}
...
async function bayarDanCetak() {
  ...
  try {
    ...
    const xItem = marginKiri;
    const xQty = lebarKertas * 0.4;
    const xHarga = lebarKertas * 0.65;
    const xSubtotal = lebarKertas - marginKanan;

    let tinggiHeader = 28;
    if (toko.logo) tinggiHeader = 40;
    ...
    doc.setFontSize(9);
    doc.text(toko.nama || 'TOKO', marginKiri, y);
    ...
    y += 4;
    doc.line(marginKiri, y, xSubtotal, y); y += 3;
    ...
    doc.text('Rp' + total.toLocaleString('id'), xSubtotal, y, { align:'right' }); y += 5;
    doc.text('Bayar:', xItem, y);
    doc.text('Rp' + bayar.toLocaleString('id'), xSubtotal, y, { align:'right' }); y += 5;
    doc.text('Kembali:', xItem, y);
    doc.text('Rp' + kembali.toLocaleString('id'), xSubtotal, y, { align:'right' }); y += 5;
    ...
    if (bluetoothDevice && bluetoothCharacteristic) {
      ...
      // Gunakan anchor tag agar tidak diblokir
      ...
      // Setelah 500ms, coba buka dialog cetak
      setTimeout(() => {
